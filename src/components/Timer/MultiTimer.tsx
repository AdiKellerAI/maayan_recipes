...
const MultiTimer: React.FC<MultiTimerProps> = ({ isVisible, onClose }) => {
  ...
  const playAlarmSound = () => {
    try {
      ...
      // Play alarm pattern: 3 beeps, pause, 3 beeps
      for (let i = 0; i < 3; i++) {
        setTimeout(() => {
          const oscillator = audioContext.createOscillator();
          const gainNode = audioContext.createGain();
      ...
      setTimeout(() => {
        for (let i = 0; i < 3; i++) {
          setTimeout(() => {
            ...
            oscillator.frequency.value = 1000; // Higher pitch for second set
            oscillator.type = 'sine';
      ...
      if ('vibrate' in navigator) {
        navigator.vibrate([200, 100, 200, 100, 200, 500, 200, 100, 200, 100, 200]);
      }
  ...
  if (!isVisible) return (
    <>
      {/* Floating Timers Row - Always Visible */}
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-40 flex items-center space-x-2 rtl:space-x-reverse">
        {timers.filter(t => t.isRunning || t.timeLeft > 0 || t.isMinimized).map((timer) => (
          <div key={timer.id} className="flex-shrink-0">
            <div 
              className="bg-gradient-to-br from-orange-500/95 to-red-500/95 rounded-full shadow-2xl border border-orange-300/50 backdrop-blur-md cursor-pointer hover:scale-105 transition-all duration-200 p-3"
              onClick={() => {
            ...
            <div className="p-6">
              ...
              {timers.length === 0 ? (
                ...
                <div className="space-y-3">
                  <h4 className="text-lg font-semibold text-gray-900 mb-4">טיימרים פעילים</h4>
                  {timers.map((timer) => (
                    <div 
                      key={timer.id} 
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4 rtl:space-x-reverse">
                          ...
                          <div>
                            <h5 className="font-semibold text-gray-900">{timer.label}</h5>
                            {timer.timeLeft > 0 && (
                        ...
                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                          {!timer.isRunning ? (
                            <button
                              ...
                              className="p-2 bg-green-500 text-white rounded-full hover:bg-green-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200"
                              title="הפעל טיימר"
                            >
